...
namespace Abdt.Babdt.TlSharp.MTProto.Crypto
{
  public class AesEngine
  {
    ...
    private uint SubWord(uint x) => (uint) ((int) AesEngine.S[(int) x & (int) byte.MaxValue] | (int) AesEngine.S[(int) (x >> 8) & (int) byte.MaxValue] << 8 | (int) AesEngine.S[(int) (x >> 16) & (int) byte.MaxValue] << 16 | (int) AesEngine.S[(int) (x >> 24) & (int) byte.MaxValue] << 24);
    ...
    private uint[,] GenerateWorkingKey(byte[] key, bool forEncryption)
    {
      ...
      switch (num1)
      {
        ...
        case 8:
          ...
          {
            workingKey[num2 >> 2, num2 & 3] = Pack.LE_To_UInt32(key, off);
            off += 4;
            ++num2;
          }
          ...
          {
            uint num4 = workingKey[index - 1 >> 2, index - 1 & 3];
            if (index % num1 == 0)
              num4 = this.SubWord(this.Shift(num4, 8)) ^ (uint) AesEngine.rcon[index / num1 - 1];
            else if (num1 > 6 && index % num1 == 4)
              num4 = this.SubWord(num4);
            workingKey[index >> 2, index & 3] = workingKey[index - num1 >> 2, index - num1 & 3] ^ num4;
          }
          if (!forEncryption)
          {
